...
function animate(timestamp) {
  ...
  // // Traffic density
  if (typeof oasidata !== 'undefined' && curOasilocation !== null) {
    curloc = oasidata.traffic.locs[curOasilocation];
    pos = Math.round(( timestamp / 500 )% curloc.recs.length) - 1;
    currec = curloc.recs[pos];
    if (typeof currec !== 'undefined' && currec.val !== null) {
      // console.log(pos, currec, curloc.recs.length);
      curval = currec.val;
      count = groupBoxes.length - 1;
      maxTraffic = parseInt(count * (curval-curloc.min)/(curloc.max-curloc.min));
      for ( var j = 0; j < count; j ++ ) {
        groupBoxes[j].material.opacity = (j < maxTraffic) ? 1 : 0;
      }
      document.getElementById('help').innerHTML = curloc.name + ' ' + currec.year + ' ' + currec.val;
    }
  }
...
renderer.domElement.ondblclick = function(event) {
  curOasilocation = (curOasilocation == null ||
      curOasilocation == oasidata.traffic.length - 1)
      ? 0 : curOasilocation + 1;
